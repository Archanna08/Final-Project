...
for i in variable_num:
    sns.displot(data=G_Pub_Pri_Insurance, x=i, multiple="dodge", hue="Health_condition-Data_source", bins=35)
    sns.displot(data=G_HIV_tests, x=i, multiple="stack", hue="Age_group")
    sns.displot(data=G_Depression_admissions, x=i, kind="kde", hue="Year", bw_adjust=.5)
    sns.displot(data=G_All_ER_visits, x=i, hue="Year", element="step", stat='density')
    sns.displot(data=G_Authorized_meds, x=i, kind="ecdf")
